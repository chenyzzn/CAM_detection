...
input_dir="E:\\R_CAM\\"
output_dir="E:\\OCR_test\\"
if not os.path.isdir(output_dir): os.mkdir(output_dir)
output_NoRead_file="E:\\OCR_Noread_box\\"
output_SBS="E:\\OCR_SBS\\"
    ...
    for filename in os.listdir(input_dir):
        ...
        if "NOREAD" in filename: #NoReadBox
            ...
            try:
                image_path_file = input_dir + filename
                img_flow = cv2.imread(image_path_file)
                cv2.imwrite(output_SBS + filename ,img_flow)
                os.remove(input_dir + filename)
            except:
                pass   
        else:
            if filename.endswith(".jpg") and "NOREAD" not in filename:
                ...
                i+=1 
                try:
                    img0 = cv2.imread(image_path_file) #讀取圖片
                    img0 = cut_box(img0) #切箱
                    ...
                    #end=time.time()
                    print(str(img0.shape) + "辨識圖片總數= "+ str(i)+", NOREAD="+ str(j)+", 併箱="+ str(k) + ", time=" + str(t))
                except:
                    pass
    task()
